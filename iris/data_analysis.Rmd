...
데이터 위치[iris](https://github.com/haven-jeon/reproducible-data-analysis-examples/blob/master/iris/iris.csv)는 될수 있는한 웹으로 접근 가능한 파일서버나 git에 올려두어 원격으로 패칭해서 문서생성 및 코드생성이 가능하게 한다. 
...
```{r fig1,fig.width=7, fig.height=6, tidy=TRUE,message=FALSE}
library(ggplot2)
...
```{r fig2,fig.width=10,fig.height=6, message=FALSE}
